import {Color, defined, ShaderSource} from '@cesium/engine';

/**
 * @private This is a clone of Cesiums ShadowMapShader.js with minor changes, see //EDIT tag
 */
...
ShadowMapShader.createShadowReceiveFragmentShader = function (
    ...
    if (isPointLight) {
        ...
        if (isTerrain && viewshed) {
            fsSource +=
                'float distance = length(directionEC);' +
                'float distFactor = clamp(distance / shadowMap_lightPositionEC.w, 0.0, 1.0);' +
                'nDotL = mix(0.13, nDotL, distFactor);\n';
        }
    ...
    if (viewshed) {
        ...
        {
            vec4 _shadowColor = vec4(${shadowColor.join(',')});
            out_FragColor.rgb = mix(out_FragColor.rgb, _shadowColor.rgb, 1.0 - _shadowColor.a);
        } else {
            vec4 _lightColor = vec4(${visibleColor.join(',')});
            out_FragColor.rgb = mix(out_FragColor.rgb, _lightColor.rgb, 1.0 - _lightColor.a);
            }
        }
        `;
        //fsSource += 'out_FragColor.rgb = vec3(visibility);}';
    } else if (
