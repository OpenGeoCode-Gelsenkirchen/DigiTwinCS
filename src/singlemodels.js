...
import './Components/GeFileUpload/GeFileUpload.js';
...
function rotateModel(direction) {
    ...
    let stepsize =
        Gizmo.rotationSnap > 0 ? Number(Gizmo.rotationSnap) : ROTATION_STEPSIZE;

    if (direction === 'ccw') {
        stepsize *= -1.0;
    }

    const rot = Matrix3.fromRotationZ(CesiumMath.toRadians(-stepsize));
...
async function moveDirection(direction) {
    ...
    const stepsize =
        Gizmo.translationSnap > 0
            ? Number(Gizmo.translationSnap)
            : MOVE_STEPSIZE;
