...
export class FirstPersonState extends State {
    ...
    async apply(app) {
        ...
        const compass = document.querySelector('GeCompass');
        if (compass) compass.clickable = false;
        ...
        this.pedestrianWindow = WindowFactory.createGeInformationWindow({
            id: 'pedestriandiv',
            title: i18next.t('common:body.pedestrian.title'),
            subtitle: i18next.t('common:body.pedestrian.subtitle'),
            message: i18next.t('common:body.pedestrian.content'),
        });

        this.pedestrianWindow.show();
    ...
    remove(app) {
        ...
        this.pedestrianWindow?.close();
        Flags.walking = false;

        const compass = document.querySelector('GeCompass');
        if (compass) compass.clickable = true;
