...
import './Components/GeBar/GeBar.js';
import './Components/GeBaseLayerPicker/GeBaseLayerPicker';
import './Components/GeButton/GeButton.js';
import './Components/GeCard/GeCard.js';
import './Components/GeCollapse/GeCollapse.js';
import './Components/GeGrid/GeGrid.js';
import './Components/GeInputField/GeInputField.js';
import './Components/GeList/GeList.js';
import './Components/GeListItem/GeListItem.js';
import './Components/GeShadowControl/GeShadowControl.js';
import './Components/GeSlider/GeSlider.js';
import './Components/GeWindow/src/GeWindow.js';
...
window.PointMeasurement = PointMeasurement;
...
//);
//const cam_newpos = Cartographic.fromCartesian(
//cam_newpos_zeroheight,
//);
...
document.getElementById('waterLevelButton').addEventListener('click', () => {
    app.waterLevel.show = !app.waterLevel.show;
});
...
document.getElementById('waterlevel-slider').addEventListener('value', v => {
    app.waterLevel.height = v.detail;
});
...
addEventListener('updateAlpha', event => {
    const alpha = event.detail;

    updateAlpha(alpha);
});

addEventListener('cesium:shadow-toggle', event => {
    if (event.detail.enabled) updateAlpha(1.0);
});
...
export function createExcavationPitFromGeojson(featureCollection) {
    let epsg;
    if (featureCollection.crs?.properties?.name) {
        epsg = featureCollection.crs.properties.name.match(/\d+/)[0];
    }
    for (const feature of featureCollection.features) {
        let cartesians = feature.geometry.coordinates[0];
        if (epsg && epsg !== 4326) {
            cartesians = cartesians.map(c => proj4('COORD', 'EPSG:4326', c));
        }
        cartesians = cartesians.map(c => Cartesian3.fromDegrees(...c));
        ...
        if (upload.files.length > 0) {
            for (const file of upload.files) {
                ...
                reader.onload = event => {
                    ...
                    createExcavationPitFromGeojson(featureCollection);
                };
        ...
        .map(item => {
            ...
            switch (true) {
                ...
                case item.obj instanceof HeightMeasurement:
                    mode = 'height';
                    break;
                case item.obj instanceof PointMeasurement:
                    mode = 'point';
                    break;
            }
