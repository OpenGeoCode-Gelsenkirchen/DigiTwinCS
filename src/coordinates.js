 ...
 * @param {CustomEvent} e - CustomEvent with detail: { position: Cartesian3 }
 *
 ...
 */
app.handler.events.addEventListener('viewer-mouse-move-3d', async e => {
    if (!window.coordinateDisplay) {
        return;
    }
    Cartesian3.clone(e.detail.position, cartesian);

    //only execute if cursor is on globe and not atmosphere/space
    if (cartesian) {
        Cartographic.fromCartesian(cartesian, Ellipsoid.default, cartographic);
