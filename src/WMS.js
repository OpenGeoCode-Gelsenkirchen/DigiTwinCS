...
import {WMSTree} from './Components/GeTree/WmsTree.js';
...
import {WMSParser} from './Core/WMSParser';
 ...
 * @returns {Promise<void>}
 */
async function addWMS(tree, url) {
    const values = tree.checked;

    for (let i = 0; i < values.length; i++) {
        const guid = values[i].name;

        if (layerCollection.getLayerById(guid)) {
            continue;
        }
...
async function createLoadingWindow() {
    ...
    const layerTree = await parser.getLayerTree(url);

    const tree = new WMSTree();
