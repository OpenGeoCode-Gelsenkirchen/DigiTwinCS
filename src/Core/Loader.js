...
export async function loadB3DMLayer(app, options = {}) {
    ...
    const credit = merged.credit
        ? new Credit(i18next.t(merged.credit))
        : undefined;
...
export async function loadB3DM(app, options) {
    ...
    //add to UI table
    if (table === true) {
        //hotfix: will do better in the future. I promise!
        const tableId = '#D3Table';
        addEntryToTable(tableId, layerName, lc.id, undefined, false);
...
import {ClippingPolygonCollection, HeightReference} from '@cesium/engine';
...
export async function addGeoJson(app, options = {}) {
    ...
    const style = options.labelConfiguration;

    if (options.type === 'label') {
        labels = app.viewer.scene.primitives.add(
            new LabelCollection({
    ...
    if (style?.style) {
        style.style = LabelStyle[style.style];
    }

    if (style?.scaleByDistance) {
        const sbd = style.scaleByDistance;
        style.scaleByDistance = new NearFarScalar(
            sbd.near,
            sbd.nearValue,
            sbd.far,
            sbd.farValue,
    ...
    if (style?.distanceDisplayCondition) {
        ...
        style.distanceDisplayCondition = new DistanceDisplayCondition(
            ddc.near,
            ddc.far,
        );
    }
    ...
    if (style?.pixelOffset) {
        style.pixelOffset = new Cartesian2(
            style.pixelOffset[0],
            style.pixelOffset[1],
        );
    }
    ...
    if (style?.fillColor) {
        style.fillColor = Color.fromCssColorString(style.fillColor);
    }
    ...
    if (style?.disableDepthTestDistance) {
        style.disableDepthTestDistance = Number(style.disableDepthTestDistance);
    }
    ...
    for (const entity of dataSource.entities.values) {
        ...
        if (options.type === 'label') {
            ...
            if (entity.polygon) {
                ...
                const sample = (
                    await safeSampleTerrainMostDetailed(
                        app.viewer.scene.terrainProvider,
                        [Cartographic.fromCartesian(bs.center)],
                    )
                )[0];
                position = new ConstantPositionProperty(
                    Cartographic.toCartesian(sample),
                );
                entity.polygon = undefined;
            }
            ...
            labels.add({
                ...style,
                position: position.getValue(JulianDate.now()),
                text: entity.properties[style.labelText].getValue(
                    JulianDate.now(),
                ),
            });
        }
    ...
    if (labels) {
        lc = new LayerCollection(app.viewer, {
            name: dataSource.name,
            type: Layer.LayerTypes.GEOJSON,
            show: show,
            onShowChange: credit
    ...
    if (table === true) {
        ...
        cb.addEventListener('change', function () {
            switchCheckbox(app, Variables.hideIDs, cb, lc || layer);
        });
    }
    return layer;
}
...
export async function loadModel(app, options = {}) {
    const {
        url,
        position,
        hpr = [0, 0, 0],
    ...
    const shadowMode =
        shadows in ShadowMode ? ShadowMode[shadows] : ShadowMode.ENABLED;

    const transform = Transforms.eastNorthUpToFixedFrame(
        projectCoordToCartesian(position),
    );

    const headingPitchRoll = new HeadingPitchRoll(
        ...hpr.map(x => CesiumMath.PI - CesiumMath.toRadians(x)),
    );

    const rotMatrix = Matrix3.fromHeadingPitchRoll(headingPitchRoll);
    const rotationMat4 = Matrix4.fromRotationTranslation(rotMatrix);
    const scaleMat4 = Matrix4.fromScale(new Cartesian3(...scale));

    const modelMatrix = Matrix4.multiply(
        transform,
        Matrix4.multiply(rotationMat4, scaleMat4, new Matrix4()),
        new Matrix4(),
    );
    ...
    if (table === true) {
        addEntryToTable('#D3Table', layerName, layer.id, undefined, false);
...
export async function loadMesh(app, options) {
    ...
    const credit = options.credit ? new Credit(options.credit) : undefined;
