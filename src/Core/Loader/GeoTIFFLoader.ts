 ...
 */
export async function loadTiffFromSource(source: TIFFSource): Promise<GeoTIFF> {
...
export async function loadTiff(
    ...
    const bbArray = image.getBoundingBox();
    const code =
...
export async function getTiffProvider(
    ...
    try {
        ...
        if (code !== 4326) {
            const win = WindowFactory.createGeWarningWindow({
                title: i18next.t('error:reprojection.title'),
                message: i18next.t('error:reprojection.text'),
            });
            win.show();
        ...
        WindowFactory.createGeErrorWindow({
