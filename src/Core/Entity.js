...
export class PointEntity extends Entity {
    ...
    constructor(
        ...
        if (position.equals(Cartesian3.ZERO)) {
            //console.log(this.app.handler.lastValidPickedPosition);
            //this.position = this.app.handler.lastValidPickedPosition;
            //REFACTOR
            this.app.handler.events.dispatchEvent(
                new CustomEvent('viewer-mouse-move-3d', {
                    detail: {
                        position: this.app.handler.lastValidPickedPosition,
                        feature: this.app.handler.lastValidPickedFeature,
                    },
                }),
            );
        }

        this._handler = EventHandler.instance;
    }
    ...
    addEventListeners() {
        if (this.selectable) {
            this.leftDown3dHandler = event => {
                if (this.primitive.id === event.detail.feature?.id?.id) {
                    this.isBeingHeld = true;
                    this.app.viewer.scene.screenSpaceCameraController.enableRotate = false;
                    this.app.viewer.selectedEntity = undefined;
                    this._handler.overrideDragging = true;
                }
            };

            this.app.handler.events.addEventListener(
                'viewer-left-down-3d',
                this.leftDown3dHandler,
                true,
            );
            ...
            this.mouseMove3dHandler = event => {
                this.mouseOver =
                    this.primitive.id === event.detail.feature?.id?.id ||
                    this.isBeingHeld;

                if (this.isBeingHeld) {
                    this.lastPickedPosition =
                        event.detail.position &&
                        !event.detail.position.equals(Cartesian3.ZERO)
                            ? event.detail.position
                            : this.lastPickedPosition;
                    this.updatePosition(this.lastPickedPosition);
                }
            };
            ...
            this.leftUpHandler = () => {
                if (this.isBeingHeld) {
                    ...
                    this.app.viewer.selectedEntity = undefined;
                    this._handler.overrideDragging = false;
                }
            };
            this.app.handler.events.addEventListener(
                'viewer-left-up',
    ...
    removeEventListeners() {
        this.app.handler.events.removeEventListener(
            'viewer-left-down-3d',
            this.leftDown3dHandler,
            true,
        );
