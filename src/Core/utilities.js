...
export function bindHold(el, fire, intervalMs) {
    let id = null;
 ...
 * @param {Array<Cartographic>} terrainSamplePositions
...
export async function makeShareLink(app) {
    ...
    const info = WindowFactory.createGeInformationWindow({
        title: i18next.t('common:body.share-view.title'),
        message: i18next.t('common:body.share-view.content'),
    });
    //info.setTimer(4);
    info.show(4);
}
 ...
 * Utility function to create the UI html table element (2D)
 */
export function createTable_label() {
    const datatable = document.getElementById('labelTable');
    const tbody = datatable.createTBody();
    tbody.style =
        'display: block; max-height: 200px; overflow-y: auto; font-size: 14px; font-style: italic; width: 336px;';
}
...
export async function detectKTX2Support() {
