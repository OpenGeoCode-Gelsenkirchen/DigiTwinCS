...
export class Layer {
    ...
    constructor(
        ...
        this._allowOpacityChange = allowOpacityChange;
        ...
        if (content && content.customShader) {
            this.originalTranslucencyMode =
                content.customShader.translucencyMode;
        }
    ...
    set customShader(cs) {
        ...
        if (this.type !== GEOJSON && this.type !== GEOJSON3D) {
            if (shader) {
                let translucencyMode = this.originalTranslucencyMode;

                if (this._allowOpacityChange && this.opaque) {
