...
type TFunction = (key: string) => string | Promise<string>;
...
export class Translator {
    static NS_SEP = ':';
    tag: string;
    isNsKey = (s: string) => s.includes(Translator.NS_SEP);
    ...
    async translate(element: Element) {
        ...
        for (const attr of attrs) {
            ...
            const translated = await this.t(path);
    ...
    translateConfig(entry: unknown): unknown {
        if (Array.isArray(entry)) {
            return entry.map(e => this.translateConfig(e));
        }
        if (entry !== null) {
            if (typeof entry === 'object') {
                ...
                return Object.fromEntries(
                    Object.entries(obj).map(([key, value]) => {
                        const translatedKey = this.isNsKey(key)
                            ? i18next.t(key, {
                                  defaultValue: key,
                              })
                            : key;
                        return [translatedKey, this.translateConfig(value)];
                    }),
                );
            } else if (typeof entry === 'string') {
                return this.isNsKey(entry)
                    ? i18next.t(entry, {defaultValue: entry})
                    : entry;
            }
        }
        return entry;
